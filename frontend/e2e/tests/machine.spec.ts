...
test.describe('Machine page', () => { // กลุ่ม test สำหรับ machine page
  test.beforeEach(async ({ request, baseURL }) => { // รันทุกครั้งก่อนที่จะทำ test
    await deleteE2eMachines(request, baseURL!); // ลบเครื่องจักรที่สร้างขึ้นมาในการทดสอบที่มีชื่อขึ้นต้นด้วย 'E2E-'
  });

  test('shows empty state for admin', async ({ authenticatedPage: page }) => { // test สำหรับแสดงสถานะว่างสำหรับ admin
  ...
  test('admin sees add button', async ({ authenticatedPage: page }) => { // test สำหรับแสดงปุ่มเพิ่มเครื่องจักรสำหรับ admin
    await page.goto('/machine'); // ไปหน้า machine

    await expect(page.getByRole('button', { name: 'เพิ่มเครื่องจักร' })).toBeVisible(); // ตรวจสอบว่ามีปุ่มเพิ่มเครื่องจักร
  });

  test('user cannot see add button', async ({ page }) => { // test user ทั่วไปไม่เห็นปุ่มเพิ่มเครื่องจักร
  ...
  test('admin can create a machine', async ({ authenticatedPage: page, request, baseURL }) => { // test สำหรับสร้างเครื่องจักรสำหรับ admin

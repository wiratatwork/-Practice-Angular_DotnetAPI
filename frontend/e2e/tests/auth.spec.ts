...
test.describe('Authentication', () => { // กลุ่ม test สำหรับ authentication
  test('redirects unauthenticated user to login', async ({ page }) => { // test สำหรับ redirect ผู้ใช้ที่ไม่ได้รับการยืนยันไปหน้า login
    await page.goto('/'); // ไปหน้า /
    await expect(page).toHaveURL(/\/login/); // ตรวจสอบว่าเป็นหน้า login
  });

  test('admin login success', async ({ page }) =>  {  // test สำหรับ login ผู้ใช้ที่เป็น admin
  ...
  test('shows error on invalid credentials', async ({ page }) => { // test สำหรับแสดง error ที่ไม่ถูกต้อง
  ...
  test('logout redirects to login', async ({ page }) => { // test สำหรับ logout ไปหน้า login
    await loginAs(page, 'admin'); // login
